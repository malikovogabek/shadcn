...
export const EvidenceTable: React.FC<EvidenceTableProps> = ({ 
  ...
  let filteredEvidence = user?.role === 'tergovchi' 
  ...
  // Apply status filter
  if (statusFilter !== 'all') {
    filteredEvidence = filteredEvidence.filter(e => e.status === statusFilter);
  }
          ...
          <TableBody>
            {filteredEvidence.map((evidence) => (
              <TableRow key={evidence.id}>
                ...
                <TableCell className="font-medium">{evidence.evidenceNumber}</TableCell>
                <TableCell className="max-w-xs truncate">{evidence.eventDetails}</TableCell>
                <TableCell>
                  <div className="flex items-center space-x-2">
                    <span>{evidence.storageType === 'lifetime' ? 'Umrbod' : evidence.storageDeadline}</span>
